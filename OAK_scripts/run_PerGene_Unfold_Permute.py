...
def run_per_gene(v_test, yvalls_test):
    ...
    for gene in np.unique(np.intersect1d(v_test["NearestGene"], yvalls_test["NearestGene"])):
        if c%1000 == 0:
            print(c)
        vk = v_test.loc[gene].copy()
        vvk = yvalls_test.loc[gene].copy()
        if len(vk.index) >= 100 and len(vvk.index) >= 50:
            ...
            for cuttt in [0.5, 0.6, 0.7, 0.8, 0.9]:
                ...
                #print([gene, np.median(x2), len(x2), np.median(yvals2), len(yvals2), fisher_exact(table1)[1], mwu(x2, yvals2)[1], alpha[0], alpha[1], table1, cuttt])
            #shuffle_alphas = []
            #for i in range(1000):
                #if i % 100 == 0:
                    #print(i)
...
if metric == "PhyloP447" or metric == "CNEP":
    ...
    df.to_csv("PerGene_Unfold_Permute/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_" + metric + "_MAFCut" + str(maf_cut) + "_SpecSup250_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    ...
    pc = -1
    df = run_per_gene(v, yvalls)
    df.to_csv("PerGene_Unfold_Permute/Finalizing_PerGene_FilterNewTEs_FisherExact_NoPseudosNoRepeats_NonCod_" + metric + "_MAFCut" + str(maf_cut) + "_SpecSup250_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    ...
    try:
        os.mkdir("PerGene_Unfold_Permute/" + dl_prefix)
    except:
        pass
    
    df = run_per_gene(v, yvalls)
    df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_All_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    
    df = run_per_gene(v[v["Position"].isin(dl_fixed.index)].copy(), yvalls[yvalls["Position"].isin(dl_poly.index)].copy())
    df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_Top20th_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    
    #v_low = v[v["SpecSup447"] < 105].copy()
    #yvalls_low = yvalls[yvalls["SpecSup447"] < 105]
    #df = run_per_gene(v_low, yvalls_low)
    #df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_All_MAFCut" + str(maf_cut) + "_SpecSupLT105_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    
    #df = run_per_gene(v_low[v_low["Position"].isin(dl_fixed.index)].copy(), yvalls_low[yvalls_low["Position"].isin(dl_poly.index)].copy())
    #df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_Top20th_MAFCut" + str(maf_cut) + "_SpecSupLT105_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    ...
    #    df = run_per_gene(v, yvalls)
    #    df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_All_AbsGreater0.1_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
        
    #    df = run_per_gene(v[v["Position"].isin(dl_fixed.index)].copy(), yvalls[yvalls["Position"].isin(dl_poly.index)].copy())
    #    df.to_csv("PerGene_Unfold_Permute/" + dl_prefix + "/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_Dl" + dl_prefix + "_" + metric.replace(" ", "_") + "_Top20th_AbsGreater0.1_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
elif metric == "BGS":
    df = run_per_gene(v, yvalls)
    df.to_csv("PerGene_Unfold_Permute/Finalizing_PerGene_FilterNewTEs_FisherExact_NonCod_" + metric + "_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
    ...
    df.to_csv("PerGene_Unfold_Permute/Finalizing_PerGene_FilterNewTEs_FisherExact_NoPseudosNoRepeats_NonCod_" + metric + "_MAFCut" + str(maf_cut) + "_SpecSup0_PhyloPCut" + str(phylop_cut) + "_PhastConsCut" + str(pc) + ".csv", index = False)
