...
dl_fixed = pd.read_csv(dl_path + dl_prefix + "_ChimpDerived" + suff + ".txt", sep = "\t")
...
#Use human polymorphism data for neutral background
bleh, vv = read_noncoding_data_fast(path = "./", maf_cut = 0.25, spec_sup = 0)
bleh = 0
...
if swap_neur:
    cts_fixed = pd.read_csv(dl_path + dl_prefix + "_EEs_ChimpDerived" + suff + "SwapNeur.txt", sep = "\t").set_index("Position")
#Original files
else:
    cts_fixed = pd.read_csv(dl_path + dl_prefix + "_EEs_ChimpDerived" + suff + "_WithNeuron.txt", sep = "\t").set_index("Position")
...
#Find RAGs and cRALs as described in the methods
print(v_bl.shape, v_bg.shape)
done = 0
out = []
...
df.to_csv("RCCTAGs/" + dl_prefix + "_RCCTAGs" + suff + "_AddSumLFC.txt", sep = "\t", index = False)
...
df = df.drop_duplicates("Positions")
df.to_csv("RCCTAGs/" + dl_prefix + "_RCCTALs" + suff + "_AddSumLFC.txt", sep = "\t", index = False)
