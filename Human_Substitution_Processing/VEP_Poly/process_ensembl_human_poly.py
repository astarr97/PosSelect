...
c = 0
out = open("All_Summarized_HumPoly.txt", 'w')
out.write("\t".join(["Chrom", "Pos", "ID", "REF", "ALT", "Variant_type", "Gene", "Gene_type", "protein_pos", "protein_change", "APPRIS", "SIFT", "PolyPhen"]) + "\n")
for i in range(83):
    o = open("HP_Ens_" + str(i) + "_Filt.vcf")
    for line in o:
        if "#" not in line:
            c += 1
            l = line.replace("\n", "").split("\t")
